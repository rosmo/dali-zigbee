...
extern const uint8_t server_cert_pem_start[] asm("_binary_letsencrypt_pem_start");
extern const uint8_t server_cert_pem_end[] asm("_binary_letsencrypt_pem_end");

static SemaphoreHandle_t ota_ready_sem;
...
static void ota_task(void *pvParameter)
{
    ...
    esp_http_client_config_t config = {
        .url = CONFIG_DALIZB_UPDATE_URL,
        //.timeout_ms = 10000,
        .crt_bundle_attach = esp_crt_bundle_attach,
        .keep_alive_enable = true,
    };

#ifdef CUSTOM_CA_CERT
    ...
    while (retries < 30) {
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to open HTTP connection (retrying): %s", esp_err_to_name(err));
            retries++;
    ...
    if (retries == 5) {
        ESP_LOGE(TAG, "Failed to open HTTP connection (fatal): %s", esp_err_to_name(err));
        esp_http_client_cleanup(client);
        xSemaphoreGive(ota_ready_sem);
        task_fatal_error();
    }
...
void over_the_air_update(bool provision)
{
    ...
    if (esp_ota_get_state_partition(running, &ota_state) == ESP_OK) {
        if (ota_state == ESP_OTA_IMG_PENDING_VERIFY) {
            ESP_LOGI(TAG, "Canceling firmware rollback and continuing...");
